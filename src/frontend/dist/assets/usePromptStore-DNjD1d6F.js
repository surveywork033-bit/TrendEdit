import { c as createLucideIcon, d as create } from "./index-CuhmJBMl.js";
/**
...
const usePromptStore = create((set, get2) => ({
  prompts: [],
  isLoading: false,
  activeFilter: "all",
  searchQuery: "",
  favorites: loadFromStorage("promptvault_favorites", []),
  copyHistory: loadFromStorage("promptvault_history", []),
  fetchPrompts: async () => {
    set({ isLoading: true });
    try {
      const { data, error } = await supabase.from("prompts").select("id, title, image_url, prompt_text, category, created_at").order("created_at", { ascending: false });
      if (error) {
        console.error(
  ...
  toggleFavorite: (id) => {
    const favorites = get2().favorites.includes(id) ? get2().favorites.filter((f) => f !== id) : [...get2().favorites, id];
  ...
  addToCopyHistory: (prompt) => {
    ...
    const existing = get2().copyHistory.filter((e) => e.id !== prompt.id);
  ...
  addPrompt: async (data) => {
    ...
    if (error) {
      ...
      throw new Error(
        error.code === "42501" || ((_a = error.message) == null ? void 0 : _a.includes("row-level security")) ? "Permission denied. Check Supabase RLS policies — enable INSERT for anon role on the prompts table." : `Failed to save prompt: ${error.message}. Check Supabase RLS policies if this persists.`
      );
    }
    await get2().fetchPrompts();
  },
  updatePrompt: async (id, updates) => {
    const { title, image_url, prompt_text, category } = updates;
    const { error } = await supabase.from("prompts").update({ title, image_url, prompt_text, category }).eq("id", id);
    if (error) {
      console.error(
